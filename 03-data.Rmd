...
このテキストでは，以降の章の解析に必要なパッケージ関数についてに絞って解説する。詳細はtidyverseの公式ページを参照のこと。
[https://www.tidyverse.org](https://www.tidyverse.org)

***
注意：  
以降のプログラムでは，関数を「`XXXX::YYYY`」と表現しているが，これらは「`XXXX`パッケージに入っている`YYYY`という関数を使う」ということを意味する。`XXXX::`の部分は，基本的に省略しても問題ないが，例えば`tidyverse`パッケージ以外も使っていて，同じ名前の関数が別のパッケージに含まれている場合には，エラーが表示されてしまう（Rがどの関数を使えばよいのかが区別できなくなってしまう）。外部パッケージの関数を使う場合は，できる限り`XXXX::`を付けて使うことにした方が無難である。    
...
上の例では，`Sepal.Length`と`Petal.Length`を足した`new_var`という名前の新しい変数を作っっている。更に，「`Sepecies`が"`setosa`"ならば1, そうでなければ0とする」という条件で新たに`hoge`という変数を作っている。
...
`iris`データで「あやめの種類のうち`"setosa"`を取り出して，かつ変数として`Species`，`Sepal.Width`, `Sepal.Length`のみを取り出したい」という複数の処理をする場合を例として考える。  
先程まで学んだ知識を使えば，以下のように複数のプラグラムを段階的に書けばできなくはないが，非常に面倒くさくなる（プログラムを分けて書くと途中でミスも生じやすくなる）。
...
Rでデータ解析に使う関数は，「1オブザベーションにつき1行」が原則，つまりlong型でデータが入っていることを想定として作られている。このテキストで学ぶデータ解析も，基本的に分析で使うデータはlong型を前提とする。  
  
＊オブザベーション（observation）：観測値，解析に使う変数を意味する。
  
その一方，データを入力する際にはwide型で用意する方が楽な場合もある。データ入力は研究者の都合に応じてやりやすい方法で用意するとして，解析をする際ときに必要なデータ形式に変換するすべも身に着けて行う。  
...
dat_wide2 = dat_long %>% tidyr::spread(key = Condition, value = Score) dat_wide2
