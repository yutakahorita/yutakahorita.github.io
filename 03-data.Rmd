...
まず，`tidyverse`パッケージを使うためにパッケージのロードをしよう。初めて使う場合は，マシンに予めパッケージをインストールする必要がある。パッケージのインストール及びロードについては，第2章で解説している。
...
`tidyverse`は，様々なパッケージを含んだパッケージのセットである。Rを使いやすくするための便利なパッケージがまとめて入っている。例えば，以下のパッケージが含まれている。  
...
このテキストでは，以降の章の解析で必要なパッケージ及び関数のみに絞って解説する。その他の詳細は`tidyverse`の公式ページを参照のこと。  
...
Rでデータ解析に用いる関数のほとんどは，「**1つの観測値（observation）につき1行**」が原則，つまりlong型でデータが入っていることを前提として作られている。このテキストで学ぶデータ解析も，基本的に分析で使うデータはlong型を前提とする。  
