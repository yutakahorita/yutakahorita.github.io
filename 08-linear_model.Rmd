...
まずは，「線形予測子」，「傾き」，「切片」といったキーワードを覚えておく。  
...
* 係数がプラスならば，係数の値が増えると従属変数の値が増加する関係にあることを意味する。  

* 係数がマイナスならば，係数の値が増えると従属変数が減少する関係にあることを意味する。  
...
ggplot() +
  ...
  geom_vline(xintercept = result$maximum, linetype="dashed", colour="red", size=1) +
  labs(x ="theta", y= "log L") 

```

対数尤度が最も大きくなるのは，$\theta=$ `r round(result$maximum, 2)`のときである（表が出た割合である2/5と一致）。  
...
***
線形モデルのときのパラメータ推定には，**最小二乗法**と呼ばれる別の推定方法もある。ただし，最小二乗法を使っても最尤法を使っても，線形モデルの傾きや切片の推定結果は全く同じになる。
