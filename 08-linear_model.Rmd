...
切片の値は`r as.numeric(round(result$coefficients[1],2))`となっているが、`Sepal.Lenght`の値がゼロのときの`Petal.Length`の予測値を意味している（この例ではアヤメのがくの長さとしてありえない負の値になっているが、これは変数の標準化等を行っていないためである。詳しくは次章で解説する）。  
