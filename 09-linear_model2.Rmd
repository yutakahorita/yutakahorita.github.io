...
式（3）より，切片の推定値は$x_{t1}=0$かつ$x_{t2}=0$のときの$\hat{y}$，つまり統制群(ctrl)のときの応答変数$y$の推定値を意味している。各ダミー変数の係数（傾き）は，切片に加わる各条件の効果を意味している。例えば，`x_t2`の係数は0.49であるが，これは$x_{t1}=1$のとき（つまりtrt1のとき）の応答変数の予測値は， 5.03 + 0.49 = 5.52となることを示している。  
