...
ここでは，各章で出てきた関数について説明する。

***
Rでは，`?XXX`（XXXは関数やパッケージ名）と入力すると，関数のヘルプが表示される。RStudioならば，右下の画面に表示される。使い方がわからない場合やオプションに何があるかを確認したいときは，ヘルプを参照しよう（ヘルプは英語であるが）。ヘルプにはプログラムの例も示されている。

```{r, eval=FALSE}

?mean #mean関数のヘルプが表示される。
?ggplot2 #パッケージ名を入れると，パッケージのヘルプが出る。
...
変数は何があるのか，何個データが有るのかなど，データの構成をヒョ時する。
