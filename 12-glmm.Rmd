...
lambda = exp(0.01+ 0.6*x + rnorm(n=N, mean=0, sd=1))
y = rpois(n=N, lambda = lambda)
dat = data.frame(y=y, x=x, ID = 1:50)

result_poisson = glm(data = dat, y ~ 1 + x, family = poisson(link = "log"))
summary(result_poisson)

performance::check_overdispersion(result_poisson) #過分散のチェック
...
result_poisson_glmm = glmer(data = dat, y ~ 1 + x + (1|ID), family = poisson(link = "log"))
